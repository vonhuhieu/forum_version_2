  ...
  <div class="forum-home">
    ...
    <section v-for="group in activeGroups" :key="group.id" :id="'group-' + group.id" class="forum-section card">
      ...
      <div class="category-list">
        <div v-for="cat in group.categories.filter(c => !c.parentCategoryId)" :key="cat.id" class="category-row home-category-row min-height-100-on-pc" @click="handleCategoryRowClick($event, cat)">
            ...
            <div v-if="lastThreadByCat[cat.id] && (cat.threadCount || 0) > 0" class="last-thread-box home-last-thread-box">
              ...
              <div class="last-thread-info home-last-thread-info">
                <router-link :to="lastThreadByCat[cat.id].lastPostId ? { name: 'ThreadDetail', params: { id: lastThreadByCat[cat.id].id }, query: { postId: lastThreadByCat[cat.id].lastPostId } } : { name: 'ThreadDetail', params: { id: lastThreadByCat[cat.id].id } }" class="last-thread-title home-last-thread-title">
                  ...
                  <span v-if="lastThreadByCat[cat.id].isFollowed" title="Chủ đề đang theo dõi" style="display: inline-flex; align-items: center; vertical-align: middle; margin-left: 4px;">
    ...
    <!-- Wrapper 2 block Chú ý và Bài viết mới nhất cho Tablet / Mobile -->
    <div class="tablet-dual-blocks">
