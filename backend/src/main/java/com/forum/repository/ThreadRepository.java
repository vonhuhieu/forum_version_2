...
public interface ThreadRepository extends JpaRepository<Thread, Long> {
    ...
    @Query("SELECT t FROM Thread t WHERE (t.scope IS NULL OR t.scope <> 'INTERNAL') ORDER BY t.lastPostAt DESC")
    List<Thread> findAllPublicOrderByLastPostAtDesc();

    @Query("SELECT t FROM Thread t WHERE t.category.id = :categoryId AND (t.scope IS NULL OR t.scope <> 'INTERNAL') ORDER BY t.pinned DESC, t.lastPostAt DESC")
